...
export function GridSettingsForm() {
  const { backgroundColor, gridColor, showGrid, majorAxisStep } = useSelector(
    Model.Selectors.getSettings
  );
  ...
  const setGridColor = React.useCallback(
    (color: ColorResult) => {
  ...
  const setMajorAxis = React.useCallback(
    (value: number) => {
  ...
  return (
    <div className="grid-settings-form">
      ...
      <div className="settings-item">
        ...
        <Popover
          interactionKind={PopoverInteractionKind.CLICK}
          content={<SketchPicker color={gridColor} onChange={setGridColor} />}
        >
          <div
