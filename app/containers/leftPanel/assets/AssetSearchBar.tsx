...
export const AssetSearchBar = React.memo(function AssetTagSelect({
  ...
  const allTagKeys: SearchKey[] = React.useMemo(
    () =>
      tagIndex.all.map((tagId) => ({
        query: tagIndex.byId[tagId].name,
        type: "tag",
        tagId,
      })),
    [tagIndex]
  ...
  const searchKeyListPredicate = React.useCallback(
    (query: string, items: SearchKey[]): SearchKey[] => {
      const tagList = items
        .filter(
          (key) =>
            key.query.toLocaleLowerCase().indexOf(query.toLocaleLowerCase()) >=
            0
        )
        .slice(0, 20);
      return tagList;
  ...
  const renderItem = React.useCallback(
    (searchKey: SearchKey, { modifiers, handleClick }: ItemRendererProps) => {
      const icon =
        searchKeys.indexOf(searchKey) >= 0 ? IconNames.TICK : IconNames.BLANK;
      return (
        <MenuItem
  ...
  const renderSearchKey = React.useCallback((key: SearchKey) => {
    return (
      <>
        {key.type === "tag" && (
          <Icon
            className="asset-search-bar-tag-icon"
            icon={IconNames.TAG}
            size={12}
          />
        )}
  ...
  const onSelectSearchKey = React.useCallback(
    (item: SearchKey) => {
