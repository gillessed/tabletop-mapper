...
export const AssetPackView = React.memo(
  ({ assetPackId }: AssetPackView.Props) => {
    ...
    const assetPack = React.useMemo(
      () => assetPackIndex.byId[assetPackId],
      [assetPackIndex, assetPackId]
    );
    ...
    const filteredAssetIds = React.useMemo(() => {
      return assetPack.assetIds
        ...
        .filter(
          (asset) =>
            filter === "" ||
            asset.name.toLocaleLowerCase().indexOf(lowerCaseFilter) >= 0
        )
    ...
    const handleSaveAssetPackTitle = React.useCallback(
      (newTitle: string) => {
