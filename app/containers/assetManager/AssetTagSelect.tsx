...
export const AssetTagSelect = React.memo(function AssetTagSelect({
  ...
  const allTags: Asset.Types.Tag[] = React.useMemo(() => tagIndex.all.map((tagId) => tagIndex.byId[tagId]), [tagIndex]);
  const selectedTags: Asset.Types.Tag[] = React.useMemo(() => assetPack.tagIds.map((tagId) => tagIndex.byId[tagId]), [assetPack, tagIndex]);
  const tagListPredicate = React.useCallback((query: string, items: Asset.Types.Tag[]): Asset.Types.Tag[] => {
    if (query.length === 0) {
      return [{
        id: EmptyQuery,
        name: 'Search tags...',
      ...
      const tagList = items.filter((tag) => tag.name.toLocaleLowerCase().indexOf(query.toLocaleLowerCase()) >= 0).slice(0, 20);
      return tagList;
    }
  }, []);
  ...
  const renderItem = React.useCallback((tag: Asset.Types.Tag, { modifiers, handleClick }: IItemRendererProps) => {
    const icon =
      tag.id === EmptyQuery ? IconNames.BLANK :
        assetPack.tagIds.indexOf(tag.id) >= 0 ? IconNames.TICK :
          IconNames.BLANK;
    return (
      <MenuItem
  ...
  const onSelectTag = React.useCallback((tag: Asset.Types.Tag) => {
    if (tagIndex.byId[tag.id] == null) {
      dispatchers.assets.createAndAddTag({ tag, assetPackId: assetPack.id });
    } else {
      dispatchers.assets.addTagToAssetPack({ tagId: tag.id, assetPackId: assetPack.id });
    }
