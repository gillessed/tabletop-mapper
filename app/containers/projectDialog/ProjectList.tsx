...
export function ProjectList() {
  ...
  const [projectFiles, setProjectFiles] = React.useState<Array<Project.Types.Project | string>>([]);
  React.useEffect(() => {
    async function loadProjects() {
      setProjectsLoading(true);
      const projectDirFiles: Filer[] = await appConfig.projectsDir.readDir();
      const filteredFiles = projectDirFiles
        .filter((file) => file.getExtension() === 'ttmp');
      const promiseArray: Array<Promise<Project.Types.Project | string>> = filteredFiles.map((file) => {
        return new Promise((resolve: (result: Project.Types.Project | string) => void) => {
          file.readFile().then((contents) => {
  ...
  return (
    <div className='project-list-container'>
      <div className='project-list'>
        ...
        {!projectsLoading && projectFiles.length === 0 && <NonIdealState
        ...
        />}
        {!projectsLoading && projectFiles.length > 0 && projectFiles.map((projectOrError) => {
          if (typeof projectOrError === 'string') {
            // TODO: gcole (show error)
            return null;
          } else {
            return <ProjectListEntry key={projectOrError.id} project={projectOrError} />;
          }
        })}
      </div>
    </div>
