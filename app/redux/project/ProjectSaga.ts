...
export function* projectSaga(context: SagaContext) {
  yield all([
    ...
    takeEvery(
      Project.Actions.saveAndQuit.type,
      saveAndQuitProjectSaga,
      context
    ),
    takeEvery(Project.Actions.quitApplication.type, quitApplicationSaga),
    ...
    takeEvery(
      Model.Actions.setFeatureGeometry.type,
      setProjectRequiresSaveSaga
    ),
...
function* saveProjectSaga(context: SagaContext) {
  ...
  const asyncProject: ReturnType<typeof Project.Selectors.get> = yield select(
    Project.Selectors.get
  );
  const model: ReturnType<typeof Model.Selectors.get> = yield select(
    Model.Selectors.get
  );
